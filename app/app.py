...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def generator(self):
        ...
        self.blackacre_wb = self.excel.Workbooks.Open(
            f"{os.getcwd()}\\성적서 양식\\갑지.xls"
        )  # 갑지 양식 열기
        self.tck_template_wb = self.excel.Workbooks.Open(
            f"{os.getcwd()}\\TCK 양식\\T-{self.item_name}.xls"
        )  # TCK RAW 데이터 양식 열기
        ...
        # 갑지 시트 데이터 입력
        blackacre_sheet.Range("F11").value = self.setting["blackacre"][
            "item_name"
        ]  # DWG NO
        blackacre_sheet.Range("F12").value = self.setting["blackacre"]["grade"]  # Grade
        blackacre_sheet.Range("F13").value = self.setting["blackacre"][
            "quantity"
        ]  # Quantity
        blackacre_sheet.Range("F14").value = self.setting["blackacre"][
            "serial_no"
        ]  # Serial NO
        blackacre_sheet.Range("F15").value = self.setting["blackacre"][
            "lot_no"
        ]  # Material LOT NO
        blackacre_sheet.Range("F16").value = self.setting["blackacre"][
            "due_date"
        ]  # due date
        blackacre_sheet.Range("F17").value = self.setting["blackacre"][
            "delivery_date"
        ]  # delivery date
        blackacre_sheet.Range("E27").value = self.setting["blackacre"][
            "inspected_by"
        ]  # inspected by
        blackacre_sheet.Range("O27").value = self.setting["blackacre"][
            "approved_by"
        ]  # Approved BY

        # 갑지 시트 복사
        ws = workbook.Worksheets.Add(
            Before=workbook.Worksheets(workbook.Worksheets.Count)
        )
        ws.Name = "갑지"
        ...
        for file_name in self.data_list:
            if data_cnt == 0:
                self.inspection_wb = self.excel.Workbooks.Open(
                    f"{os.getcwd()}\\성적서 양식\\{self.item_name}.xls"
                )  # 데이터 양식 열기
            ...
            tck_sheet1.Range(self.setting["raw"]["raw_data_loc"]).PasteSpecial(
                Paste=-4163
            )  # raw data를 tck 양식에 복사
            raw_sheet.Range(
                self.setting["raw"]["serial_num_loc"]
            ).Copy()  # raw data serial num 복사
            ...
            self.inspection_sheet.Range(loc).PasteSpecial(
                Paste=-4163
            )  # raw data serial num 붙여넣기

            tck_cmm_sheet = self.tck_template_wb.Worksheets("CMM")
            tck_cmm_sheet.Range(
                self.setting["raw"]["copy_data_loc"]
            ).Copy()  # 최종 데이터 복사
